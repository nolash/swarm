...
func benchmarkPyramidHasher(b *testing.B) {
	...
	for i := 0; i < b.N; i++ {
		...
		putGetter := storage.NewHasherStore(&storage.FakeChunkStore{}, storage.MakeHashFunc(storage.BMTHash), false, chunk.NewTag(0, "foo", 0))

		ctx := context.Background()
		_, wait, err := storage.PyramidSplit(ctx, buf, putGetter, putGetter, chunk.NewTag(0, "foo", dataLength/4096+1))
		if err != nil {
			b.Fatalf(err.Error())
		}
		err = wait(ctx)
